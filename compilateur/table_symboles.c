...
int ts_addr(char * nom)
{
    int i = pos_symbole - 1;
    int addr = -1;
    while (i >= 0 && addr == -1 && table_symbole[i].niveau == niveau_courant) {
        if (strcmp(table_symbole[i].nom, nom) == 0) {
            addr = i;
        }
        i--;
    }
...
int ts_create_tmp()
{
    ...
    table_symboles[pos_symbole].type = DEFAULT;
    table_symboles[pos_symbole].is_init = VAR_INIT;
    table_symboles[pos_symbole].is_const = DEFAULT;
...
void ts_delete_tmp()
{
    ...
    table_symboles[pos_symbole].is_init = DEFAULT;
}
